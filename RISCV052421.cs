...
namespace RISCVE
{
    public class RISCV
    {
        ...
        public struct nStr
        {
            SyntaxNode node; int val;

            public nStr(SyntaxNode node, int val)
            {
                this.node = node;
                this.val = val;
            }
        ...
        public void Build(ref RISCV rISCV, string fPath, ListBox lB1in)
        {
    ...
    public class CEngineWalker : CSharpSyntaxWalker
    {
        ...
        public Stack<XVal> xPRstk = new Stack<XVal>();
        ...
        public class XVal
        {
            public XVal(SyntaxNode n, List<XVal> xValsA)
            {
                node = n;
                vix = xValsA.Count;
                Cvals = xValsA;
            }
            public XVal(SyntaxNode n, int Val)
            {
                node = n;
                vval = Val;
            }
        ...
        public override void Visit(SyntaxNode node)
        {
            ...
            XVal xV = new XVal(null, xVals);
            XVal opL = new XVal(null, 0);
            XVal opR = new XVal(null, 0);
            //RISCV.MEMORY mEMORY = rISCV.Mem;
            ...
            switch (node.Kind().ToString())
            {
                ...
                case "SimpleAssignmentExpression":
                    xPRstk.Push(xV);
                    Console.WriteLine("  visit " + node.ToString() + " nodeStk Count = " + nodeStk.Count.ToString());
                    break;
            ...
            switch (node.Kind().ToString())
            {
                ...
                case "IdentifierName":
                case "NumericLiteralExpression":
                    if ((xV = xVals.Find(vnm => vnm.node.ToString() == node.ToString())) == null)
                    {
                        xV = new XVal(node, xVals);
                        string nS = node.ToString();
                        if (node.Kind().ToString() == "NumericLiteralExpression")
                            xV.val = (int)node.GetFirstToken().Value;
                        xVals.Add(xV);
                    }
                    else
                    { }
                ...
                case "SimpleAssignmentExpression":
                    ...
                    //if ((xV = xVals.Find(vnm => vnm.node == saL)) != null)
                    //    xV.val = xPRstk.Pop().val;
                    //else
                    //    if (xVals[0].node != node)
                    //{
                    //    Console.WriteLine(node + " xV = " + xV.node);
                    //}
                    Console.WriteLine("assign " + xV.node + " = " + xV.val.ToString());
                    break;
                case "AddExpression":
                case "SubtractExpression":
                case "BitwiseAndExpression":
                ...
                case "GreaterThanOrEqualExpression":
                    ...
                    Console.WriteLine(" binary expr " + node);       //().ToString());
                    var ntL = node.ChildNodesAndTokens();  //  num, RI;  else RR
                    var oper = node.ChildTokens();
                                                           //= node.L
                                                           ...
                                                           //mEMORY.iList.Add((uint)RVE.FUNCodes.ADD);
                                                           //var f3 = mEMORY.iList[mEMORY.iList.Count - 1];
                                                           //if ((xV = xVals.Find(vnm => vnm.node == rgt)) != null)
                                                           //    opR = xV;
                                                           //else { }

                    var rt = ntL[2];
                    var lt = ntL[0];
                    //if ((xV = xVals.Find(vnm => vnm.node.ToString() == lft.ToString())) != null)
                    //    opL = xV;
                    //else
                    //{ }

                    //if (Char.IsDigit(lft.ToString()[0]))
                    //{
                    //    int lv = opL.val;
                    //}


                    //if (Char.IsDigit(rgt.ToString()[0]))
                    //{
                    //    if (opR.val > 4096)
                    //    { }
                    //}
                        ...
                        switch (ntL[1].AsToken().Text)
                    {
                            case "+":
                                opL.val += opR.vval;
                                //rISCV.Mem.iList.Add((uint)(RISCV.FUNCodes.ADD | RISCV.FUNCodes.RType));
                                break;
                            case "-":
                                opL.val -= opR.vval;
                                //        //rISCV.Mem.iList.Add((uint)(RISCV.FUNCodes.SUB | RISCV.FUNCodes.RType));
                                //        //uCwds.Add((uint)UcdEnum.sub);
                                break;
                            case "*":
                                xV.val = opL.val *= opR.val;
                                break;
                            //    case "/":
                            //        xV.val = opL.val /= opR.val;
                            //        break;
                            //    case "&":
                            //        xV.val = opL.val &= opR.val;
                            //        //rISCV.Mem.iList.Add((uint)(RISCV.FUNCodes.AND | RISCV.FUNCodes.RType));
                            //        break;
                            //    case "|":
                            //        xV.val = opL.val |= opR.val;
                            //        //rISCV.Mem.iList.Add((uint)(RISCV.FUNCodes.OR | RISCV.FUNCodes.RType));
                            //        break;
                            //    case "^":
                            //        xV.val = opL.val ^= opR.val;
                            //        //rISCV.Mem.iList.Add((uint)(RISCV.FUNCodes.XOR | RISCV.FUNCodes.RType));
                            //        break;
                            //    case "==":
                    ...
                    cwIx = uCwds.Count;
                    Console.WriteLine(" " + xV.node + " = " + xV.val);
                    xPRstk.Peek().val = xV.val;
                    break;
                case "PredefinedType":
